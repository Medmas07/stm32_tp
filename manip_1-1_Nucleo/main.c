 ...
 *Reg_Enable_RCC  |= (1<<2) | (1<<4) ;

 // Configurer les bits reliés aux leds
*Reg_Config_Led &= ~(0xF << (5 * 4));
 *Reg_Config_Led |=  (0x2 << (5 * 4)) ;
 
 // configurer le bit relié au bouton
	*Reg_Config_Bouton &= ~(0xF << ((13 - 8) * 4));
 *Reg_Config_Bouton |=  (0x4 << 20); // Input Floating
...
while (1)
{
  etat_bouton = (*Reg_Bouton & (1 << 13)) >> 13; // lire l'etat du bit relié au bouton (bit PA0)
  
if (etat_bouton == 0x0001) // si appui

{
 *Reg_BSRR_Led = (1 << 5);// allumer leds
